...
export default function ProductDetailPage() {
	...
	const fetchPopularProducts = async () => {
		try {
			...
			setPopularProducts(result.products as any || []);
		} catch (error) {
	...
	const submitReview = async () => {
		...
		try {
            ...
            const reviewRef = push(ref(db, `products/${product?._id}/reviews`));
            const newReview = {
                _id: reviewRef.key,
                user: {
                    _id: user?._id,
            ...
            const productRef = ref(db, `products/${product?._id}`);
            const productSnap = await get(productRef);
            if (productSnap.exists()) {
                const currentProduct = productSnap.val();
                const currentRating = currentProduct.rating || { average: 0, count: 0 };
                const newCount = (currentRating.count || 0) + 1;
                const newAverage = ((currentRating.average || 0) * (currentRating.count || 0) + reviewData.rating) / newCount;
                
                await set(ref(db, `products/${product?._id}/rating`), {
                    average: newAverage,
                    count: newCount
                });
            }
		...
		<div className='min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100'>
			...
			<div className='max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-8'>
				<div className='grid lg:grid-cols-2 gap-8 lg:gap-12'>
					...
					<div className='space-y-6'>
						<div>
							...
							<p className='text-slate-600 text-base sm:text-lg leading-relaxed'>{product.shortDescription || product.description}</p>
						</div>

						<div className='flex items-baseline gap-3 p-4 sm:p-6 bg-gradient-to-r from-orange-50 to-rose-50 rounded-2xl border border-orange-100'>
							<span className='text-2xl sm:text-4xl font-bold text-slate-900'>{product.currency} {product.price}</span>
							{product.comparePrice && product.comparePrice > 0 && (
								<span className='text-lg sm:text-xl text-slate-400 line-through'>{product.currency} {product.comparePrice}</span>
							)}
						</div>
						...
						<div className='flex flex-row gap-2 sm:gap-3'>
							<Button
								...
								className='flex-1 bg-gradient-to-r from-orange-500 to-rose-500 hover:from-orange-600 hover:to-rose-600 text-white py-5 px-2 text-sm sm:text-lg font-semibold shadow-lg shadow-orange-500/25 rounded-xl cursor-pointer h-auto'>
								{loading ? <Loader2 className='animate-spin mr-1 sm:mr-2' size={18} /> : <Zap className='mr-1 sm:mr-2' size={18} />}
								Buy Now
							</Button>

							<Button
								onClick={() => isInCart(product._id) ? router.push("/shop/cart") : handleAddToCart()}
								disabled={product.status !== "active"}
								variant='outline'
								className='flex-1 py-5 px-2 text-sm sm:text-lg font-semibold rounded-xl border-2 border-orange-200 text-orange-600 hover:bg-orange-50 cursor-pointer h-auto'>
						...
						{product.demoUrl && (
							<Link href={product.demoUrl} target='_blank' className='block'>
								<Button variant='ghost' className='w-full py-5 cursor-pointer px-6 rounded-xl border border-slate-200 text-slate-600 hover:bg-slate-50'>
						...
						<div className='grid grid-cols-3 gap-2'>
							<div className='flex items-center gap-2 text-sm text-slate-600'><Shield size={18} className='text-emerald-500' /><span>Secure Payment</span></div>
							<div className='flex items-center gap-2 text-sm text-slate-600'><Download size={18} className='text-blue-500' /><span>Instant Download</span></div>
							<div className='flex items-center gap-2 text-sm text-slate-600'><Zap size={18} className='text-amber-500' /><span>Lifetime Access</span></div>
						</div>
					</div>
				</div>
				...
				<div className='mt-12'>
					...
					<div className='prose prose-slate text-wrap max-w-none text-slate-600 leading-relaxed' style={{ overflowWrap: "break-word", wordBreak: "break-word" }} dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(product.description) }} />
				</div>
			</div>
			...
			<div className='max-w-7xl mx-auto px-4 md:px-6 lg:px-8 mt-16'>
				<div className='flex justify-between items-center mb-8'>
					...
					{user && <Button onClick={() => setShowReviewForm(true)} className='bg-gradient-to-r from-orange-500 to-rose-500'>Write a Review</Button>}
				</div>
				...
				{product.reviews && product.reviews.length > 0 ? (
							...
							<div key={review._id} className='bg-white border border-slate-200 rounded-xl p-6'>
								<div className='flex items-center justify-between mb-4'>
									<div className='flex items-center gap-3'>
										<div className='w-10 h-10 rounded-full bg-gradient-to-br from-slate-200 to-slate-300 flex items-center justify-center'><span className='font-semibold text-slate-700'>{review?.user?.name?.charAt(0).toUpperCase()}</span></div>
										<div>
											<p className='font-semibold text-slate-900'>{review?.user?.name}</p>
											<div className='flex items-center gap-1'>{[...Array(5)].map((_, i) => (<Star key={i} size={16} className={i < review.rating ? "text-amber-400 fill-amber-400" : "text-slate-300"} />))}</div>
										</div>
									</div>
					...
					<div className='bg-white border border-slate-200 rounded-xl p-12 text-center'><Star size={48} className='mx-auto mb-4 text-slate-300' /><h3 className='text-lg font-semibold text-slate-900 mb-2'>No reviews yet</h3><p className='text-slate-500'>Be the first to review this product</p></div>
				)}
			</div>
			...
			<Dialog open={showReviewForm} onOpenChange={setShowReviewForm}>
				<DialogContent className='max-w-md'>
					...
					<div className='space-y-4 py-4'>
						<div><label className='block text-sm font-medium text-slate-700 mb-2'>Rating</label><div className='flex gap-1'>{[1, 2, 3, 4, 5].map((star) => (<button key={star} type='button' onClick={() => setReviewData({ ...reviewData, rating: star })} className={`p-1 ${star <= reviewData.rating ? "text-amber-400" : "text-slate-300"}`}><Star size={24} fill={star <= reviewData.rating ? "currentColor" : "none"} /></button>))}</div></div>
						<div><label className='block text-sm font-medium text-slate-700 mb-2'>Review</label><textarea value={reviewData.comment} onChange={(e) => setReviewData({ ...reviewData, comment: e.target.value })} className='w-full p-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent' rows={4} placeholder='Share your experience with this product...' /></div>
						<div className='flex justify-end gap-3 pt-4'><Button variant='outline' onClick={() => setShowReviewForm(false)}>Cancel</Button><Button onClick={submitReview} disabled={reviewData.rating === 0 || reviewData.comment.trim() === ""} className='bg-gradient-to-r from-orange-500 to-rose-500 hover:from-orange-600 hover:to-rose-600'>Submit Review</Button></div>
					</div>
				</DialogContent>
			</Dialog>
